    ...
    <div class="card-body">
      ...
      <table class="table table-hover datatable table-sm">
        ...
        <tbody>
            ...
            <td><a href="" class="badge rounded-pill bg-{{ $basu->block == 0 ? 'success':'danger' }}"> @if ($basu->block == 0) <span class="text-white"><i class="bi bi-person-check-fill"></i></span> @else <span class="text-white"><i class="bi bi-person-x-fill"></i></span>  @endif</a></td>
            <td>@if ($basu->teamname)
                {{ $basu->teamname }} @else {{ __("Withdout team") }}
            @endif</td>
           ...
           @php
               ...
               $rand = Str::random(20);
           @endphp
            <td><a class="text-success badge rounded-pill bg-info" href="{{ route('blogusr.users.allteam.users.add.confirm',['sepTeamName'=>$sepTeamName,'teamId'=>$teamId,'username'=>$user_name,'user_id'=>$user_id,'rand'=>$rand]) }}">Add To My team</a></td>
          </tr>
          @endforeach
